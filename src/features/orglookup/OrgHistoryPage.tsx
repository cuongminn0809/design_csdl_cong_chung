...
import { OrgHistoryDetailModal } from "./dialogs"
import { ORG_SEARCH_LOGS, type OrgSearchLog } from "./config"
...
const HANOI_ORGS = ["VPCC Minh Anh", "VPCC Hoàn Kiếm"]
function inScope(role: HistoryRole, log: OrgSearchLog): boolean {
  if (role === "btp") return true // BR-02
  if (role === "stp") return log.scopeKey === "TP Hà Nội" || (log.scopeLevel === "tchncc" && HANOI_ORGS.includes(log.scopeKey)) // BR-01
  return log.scopeKey === "VPCC Minh Anh" // BR-03
}

const parseVn = (s: string) => {
  const [d, m, rest] = s.split("/")
  return `${rest.slice(0, 4)}-${m}-${d}`
}
...
export function OrgHistoryPage() {
  ...
  const [detail, setDetail] = useState<OrgSearchLog | null>(null)
  const [, force] = useState(0)

  const donViOptions = useMemo(() => [...new Set(ORG_SEARCH_LOGS.map((l) => l.donVi))], [])

  const filtered = useMemo(() => {
    return ORG_SEARCH_LOGS.filter((l) => inScope(role, l)).filter((l) => {
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/tra-cuu/cong-chung-vien-tchncc/to-chuc-hncc")} className="flex size-9 items-center justify-center rounded-md border border-border bg-surface text-foreground-muted shadow-xs hover:bg-surface-muted"><ArrowLeft className="size-4" /></button>
          <div>
            <h3 className="text-[22px] font-semibold leading-tight tracking-[-0.02em] text-foreground-strong">Xem lịch sử tra cứu thông tin tổ chức HNCC</h3>
            <p className="mt-1 text-[13px] text-foreground-muted">Nhật ký các lần tra cứu thông tin tổ chức hành nghề công chứng theo phạm vi dữ liệu được phép xem.</p>
          </div>
        </div>
      ...
      <div className="mt-4 overflow-hidden rounded-[14px] border border-border bg-surface shadow-sm">
        {filtered.length ? (
          ...
          <EmptyState icon={<FileSearch className="size-6" />} title="Không có dữ liệu lịch sử tra cứu tổ chức công chứng" desc="Không có nhật ký nào khớp với bộ lọc và phạm vi dữ liệu hiện tại." actionLabel="Đặt lại bộ lọc" onAction={doReset} />
        )}
      </div>

      {detail && <OrgHistoryDetailModal log={detail} onClose={() => setDetail(null)} />}
    </div>
  )
}
