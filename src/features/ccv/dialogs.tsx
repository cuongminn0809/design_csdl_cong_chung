...
import { Award, ScrollText, X } from "lucide-react"

import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { Pagination, StatusPill, Th } from "../ingestion/shared"
import { certStatusMeta, ccvStatusMeta, findCcv, type Certificate, type CcvSearchLog } from "./config"
...
function Modal({ title, icon, wide, onClose, children }: { title: string; icon?: React.ReactNode; wide?: boolean; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center bg-[rgba(10,10,10,0.5)] p-6" onClick={onClose}>
      <div className={cn("flex max-h-[88vh] w-full flex-col overflow-hidden rounded-xl bg-surface shadow-popover", wide ? "max-w-[860px]" : "max-w-[560px]")} onClick={(e) => e.stopPropagation()}>
...
/* ============================ SCR-A.5.6-02 — CHI TIẾT CHỨNG CHỈ ============================ */
export function CertificateModal({ cert, onClose }: { cert: Certificate; onClose: () => void }) {
  return (
    <Modal title="Chi tiết chứng chỉ hành nghề" icon={<Award className="size-[18px] text-foreground-muted" />} onClose={onClose}>
      <div className="grid grid-cols-1 gap-x-6 sm:grid-cols-2">
        <Row label="Số chứng chỉ" value={cert.soChungChi} />
        <Row label="Ngày cấp" value={cert.ngayCap} />
        <Row label="Nơi cấp" value={cert.noiCap} />
        <Row label="Ngày hiệu lực" value={cert.ngayHieuLuc} />
        <Row label="Ngày hết hạn" value={cert.ngayHetHan} />
        <Row label="Đơn vị cấp" value={cert.donViCap} />
        <Row label="Trạng thái" node={<StatusPill meta={certStatusMeta(cert.trangThai)} />} />
      </div>
    </Modal>
  )
}

/* ============================ SCR-A.5.6-04 — CHI TIẾT LỊCH SỬ TRA CỨU ============================ */
export function HistoryDetailModal({ log, onClose }: { log: CcvSearchLog; onClose: () => void }) {
  const [page, setPage] = useState(1)
  const [size, setSize] = useState(10)
  const viewed = log.viewedCcvIds.map((id) => findCcv(id)).filter(Boolean) as NonNullable<ReturnType<typeof findCcv>>[]
          ...
          <table className="w-full min-w-[720px] border-collapse text-[13px]">
            <thead>
              <tr className="border-b border-border bg-neutral-50">
                ...
                <Th className="px-3.5 py-2.5">Số thẻ</Th>
                <Th className="px-3.5 py-2.5">Số CCHN</Th>
                <Th className="px-3.5 py-2.5">Tổ chức hành nghề</Th>
            ...
            <tbody>
              {paged.map((c, i) => (
                <tr key={c.id} className="border-b border-neutral-100 last:border-0">
                  ...
                  <td className="px-3.5 py-2.5 text-foreground-muted">{c.soTuPhap}</td>
                  <td className="px-3.5 py-2.5 font-medium text-foreground">{c.hoTen}</td>
                  <td className="px-3.5 py-2.5 font-mono text-[12px] text-foreground-muted">{c.soThe}</td>
                  <td className="px-3.5 py-2.5 font-mono text-[12px] text-foreground-muted">{c.certificate.soChungChi}</td>
                  <td className="px-3.5 py-2.5 text-foreground-muted">{c.tchncc}</td>
                  <td className="px-3.5 py-2.5"><StatusPill meta={ccvStatusMeta(c.trangThai)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
